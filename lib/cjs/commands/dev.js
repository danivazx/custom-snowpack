...
async function startServer(commandOptions, { isDev: _isDev, isWatch: _isWatch, preparePackages: _preparePackages, } = {}) {
  ...
  - ${colors.cyan('mkcert')}: ${colors.yellow('mkcert -install && mkcert -key-file snowpack.key -cert-file snowpack.crt localhost')}
    ...
    async function loadUrl(reqUrl, { isSSR: _isSSR, isHMR: _isHMR, isResolve: _isResolve, encoding: _encoding, importMap, originalUrl, } = {}) {
        ...
        // Do not cache pages inside the `pages` folder, because they are modifed through the plugin but are not modified
        // in the code during development, this causes colision with nudl framework.
        let fileBuilder = cacheKey.includes('/pages/') ? false : inMemoryBuildCache.get(cacheKey);
        if (!fileBuilder) {
